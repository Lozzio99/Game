...
public class Matrix
{
    ...
    /**
     * static method that performs the tanh on a matrix (range <-1 , 1>)
     * @param m the given matrix
     * @return a new matrix after each value has been activated
     */
    ...
    /**
     * static method to perform a random mutation on a given matrix
     * @param m the starting matrix
     * @return a matrix with a single random position changed
     */
    public static Matrix mutate (Matrix m)
    {
        double [][] res = clone(m.getMatrix());
        for (int i = 0; i< new Random().nextInt(3);i++)
        {
            int x =  new Random().nextInt(res.length);
            int y = new Random().nextInt(res[0].length);
            res[x][y] =new Random().nextDouble();
            if(new Random().nextDouble()<0.5)
            {
                res[x][y] *=-1;
            }
